...
func TestDepBotsRenovateEnriched(t *testing.T) {
	...
	plan, err := render.BuildRecipe(l, []string{"base-layout", "dep-bots-go"}, baseDepBotsAnswers("renovate"))
...
func TestDepBotsDependabotHasGapNote(t *testing.T) {
	l := module.NewFSLoader(keel.BuiltinFS)
	plan, err := render.BuildRecipe(l, []string{"base-layout", "dep-bots-go"}, baseDepBotsAnswers("dependabot"))
	require.NoError(t, err)
