...
Thanks for contributing! `demo` is a Go module at `github.com/RomanAgaltsev/demo`.
...
`github.com/RomanAgaltsev/demo`. Adding a third-party dependency means adding it to
...
- [ ] **Branch protection** on `main`: require the pull-request checks this recipe emits —
  `lint`, `test`, `typos`, `actionlint` and `review`, plus `analyze` (CodeQL) and
